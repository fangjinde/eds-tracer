...
public class TraceDemoController {
	...
	private ExecutorService executorService = new ThreadPoolExecutor(3, 3, 0L,
	...
	private static final Logger logger = LoggerFactory
			.getLogger(TraceDemoController.class);

	@RequestMapping(path = "/webClient/httpClient")
	public ResponseView sendByHttpClient() {
		...
		try {
			...
			logger.error("sendByHttpClient error", e);
		}
		finally {
			try {
				client.close();
	...
	public ResponseView sendByRestTemplate() {
		...
		String pong = restTemplateDemo.getForObject(
				String.format("http://%s/web/echo?ping={1}", serviceId), String.class,
				"hello");
	...
	public ResponseView requestBody(@RequestBody DemoDto demoDto,
	...
	public ResponseView ddbOps() {
		...
		boolean updated = eduAttributesService.updateAttributes(
	...
	public ResponseView sendMessage(
	...
	public ResponseView searchCourse(
		...
		clauses.add(new ClauseQuerySyntax(new NDirQuery(
				new PhraseQuerySyntax("course_name_suff_sepa", keyword, false, 100f)),
				QueryOccur.SHOULD, 100f));
		clauses.add(new ClauseQuerySyntax(
				new NDirQuery(
						new PhraseQuerySyntax("provider_suff_sepa", keyword, false, 1f)),
				QueryOccur.SHOULD, 100f));

		int pageSize = 10;
		int pageIndex = 1;

		PaginationResult<StudyCourseDocument> result = studyCourseSearchDao
				.getByQueryCondition(new NDirQuery(clauses),
						new BaseQuery(pageSize, pageIndex), searchConfig);
	...
	public ResponseView redisTemplateValueSet() {
		...
		studyRedisTemplate.opsForValue().set(key1,
				new DemoDto().withId(1L).withName("name"));
		DemoDto demoDto1 = (DemoDto) studyRedisTemplate.opsForValue().get(key1);
		studyRedisTemplate.opsForValue().set(key2,
				new DemoDto().withId(2L).withName("name2"));
		List<Object> lists1 = studyRedisTemplate.opsForValue()
				.multiGet(Arrays.asList(key1, key2));
	...
	public ResponseView studyMemcachedClientOps() {
		...
		// use bareMemcachedClient because keyPrefixMemcachedClient impl error(connection
		// shutdown before call) for
		// method getBulk.
		...
		Map<String, Object> multikeysRet = bareMemcachedClient.getBulk("1", "2", "3", "4",
				"5", "6");
	...
	private void runSth() {
		String key = "ddb_trace_demo_ops_key";
		boolean updated = eduAttributesService.updateAttributes(key,
				String.valueOf(new Random().nextLong()));
	}
	...
	public ResponseView transactionMessageOps() {
		...
		String curTime = eduAttributesService
				.getAttributesValue(TransactionMessageTestContants.Key1);
	...
	public ResponseView transactionMessageResult() {
		...
		String v1 = eduAttributesService
				.getAttributesValue(TransactionMessageTestContants.Key1);
		String v2 = eduAttributesService
				.getAttributesValue(TransactionMessageTestContants.Key2);
	...
	private void addThenDelete() {
		...
		indexQuery.setObject(new DemoDocumentDto().withId(id)
				.withName("fixed index" + System.currentTimeMillis()));
		elasticsearchOperations.index(indexQuery);

		DemoDocumentDto demoDocumentDto = elasticsearchOperations.queryForObject(getQuery,
				DemoDocumentDto.class);

		elasticsearchOperations.delete(DemoDocumentDto.class, id);

		DemoDocumentDto demoDocumentDto2 = elasticsearchOperations
				.queryForObject(getQuery, DemoDocumentDto.class);
	...
	private void updateOrAdd() {
		...
		try {
			indexRequest.source(resultsMapper.getEntityMapper()
					.mapToString(new DemoDocumentDto().withId(id).withName(
							"update or add case" + System.currentTimeMillis())));
		}
		catch (IOException e) {
			e.printStackTrace();
		}
	...
	private void addOrUpdate() {
		...
		if (demoDocumentDto == null) {
			...
			UpdateRequest uRequest = new UpdateRequest();
			try {
				// uRequest.doc(jsonBuilder().startObject().field("name", "学习目标
				// 掌握java泛型的产生意义ssss").endObject());
	...
	private String singleIndex() {
		...
		indexQuery.setObject(
				new DemoDocumentDto().withId(String.valueOf(new Random().nextLong()))
						.withName("single index"));
	...
	private List<DemoDocumentDto> queryByName() {
		...
		SearchQuery searchQuery = new NativeSearchQueryBuilder()
				.withQuery(matchQuery("name", "index"))
				.withPageable(PageRequest.of(0, 10)).build();

		List<DemoDocumentDto> retList = elasticsearchOperations.queryForList(searchQuery,
				DemoDocumentDto.class);
		return retList;
	}

	private void addRecords(List<VerIndexRequest> indexDocRequestList,
			boolean addOrDelete, Long... ids) {
		for (Long id : ids) {
			...
			indexDocRequest.setDataValue("recommend_tag", "recommend_tag",
					FieldType.STRING);
			if (addOrDelete) {
