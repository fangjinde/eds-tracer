 ...
 * {@link org.springframework.cloud.sleuth.instrument.web.TraceRequestAttributes#HANDLED_SPAN_REQUEST_ATTR} when the request is processed.
 * That way the {@link TraceFilterLegacy} will not create the "fallback" span.
 *
 * @author Marcin Grzejszczak
 * @since 1.0.3
 */
public class TraceHandlerInterceptorLegacy extends HandlerInterceptorAdapter {
    ...
    public TraceHandlerInterceptorLegacy(BeanFactory beanFactory) {
    ...
    private Span getRootSpanFromAttribute(HttpServletRequest request) {
        return (Span) request.getAttribute(TraceFilterLegacy.TRACE_REQUEST_ATTR);
    }
