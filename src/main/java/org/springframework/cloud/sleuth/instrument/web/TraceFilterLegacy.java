 ...
 * {@link org.springframework.cloud.sleuth.TraceKeys}.
 *
...
public class TraceFilterLegacy extends GenericFilterBean {
    ...
    private static final Log log = LogFactory.getLog(TraceFilterLegacy.class);

    private static final String HTTP_COMPONENT = "http";

    /**
     * If you register your filter before the {@link org.springframework.cloud.sleuth.instrument.web.TraceFilterLegacy} then you will not
    ...
    protected static final String TRACE_REQUEST_ATTR = TraceFilterLegacy.class.getName()
                                                       + ".TRACE";

    protected static final String TRACE_ERROR_HANDLED_REQUEST_ATTR = TraceFilterLegacy.class.getName()
                                                                     + ".ERROR_HANDLED";

    protected static final String TRACE_CLOSE_SPAN_REQUEST_ATTR = TraceFilterLegacy.class.getName()
                                                                  + ".CLOSE_SPAN";

    private static final String TRACE_SPAN_WITHOUT_PARENT = TraceFilterLegacy.class.getName()
                                                            + ".SPAN_WITH_NO_PARENT";

    private static final String TRACE_EXCEPTION_REQUEST_ATTR = TraceFilterLegacy.class.getName()
                                                               + ".EXCEPTION";
    ...
    public TraceFilterLegacy(BeanFactory beanFactory) {
        this(beanFactory, skipPattern(beanFactory));
    }

    public TraceFilterLegacy(BeanFactory beanFactory, Pattern skipPattern) {
    ...
    private static Pattern skipPattern(BeanFactory beanFactory) {
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug(
                        "The default SkipPatternProvider implementation is missing, will fallback to a default value of patterns");
            }
        }
    ...
    private void detachOrCloseSpans(HttpServletRequest request,
        ...
        if (span != null) {
            ...
            if (httpStatusSuccessful(response) && !requestHasAlreadyBeenHandled(request)) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Will close span " + span + " since " + (shouldCloseSpan(
                            request) ? "some component marked it for closure" : "response was unsuccessful for the root span"));
                }
                handler().handleSend(response, exception, span);
        ...
        try {
            ...
            if (skip) {
                ...
                spanFromRequest = handler().handleReceive(httpTracing().tracing()
                                ...
                                HttpServletRequest::getHeader), request);
            }
            if (log.isDebugEnabled()) {
    ...
    @SuppressWarnings("unchecked") HttpServerHandler<HttpServletRequest, HttpServletResponse> handler() {
