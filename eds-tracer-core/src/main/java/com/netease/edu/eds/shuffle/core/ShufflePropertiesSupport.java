...
public class ShufflePropertiesSupport {
	...
	public static int getDelayMSToSendLatter() {
		ShuffleProperties shuffleProperties = SpringBeanFactorySupport
				.getBean(ShuffleProperties.class);
	...
	public static long getTestEnvQueueExpirePeriod() {
		...
		if (environment != null) {
			testEnvQueueExpirePeriodValue = environment
					.getProperty("shuffle.testEnvQueueExpirePeriod");
		}
	...
	public static long getTestEnvQueueMessageTtl() {
		...
		if (environment != null) {
			testEnvQueueMessageTtlValue = environment
					.getProperty("shuffle.testEnvQueueMessageTtl");
		}
	...
	public static Map<String, String> getHttpClientHostRewriteTo() {
		ShuffleProperties shuffleProperties = SpringBeanFactorySupport
				.getBean(ShuffleProperties.class);
		if (shuffleProperties == null) {
		...
		List<ShuffleProperties.HttpClientRewriteRule> httpClientRewriteRules = shuffleProperties
				.getHttpClientRewriteRules();

		Map<String, String> map = new LinkedHashMap<>();
		httpClientRewriteRules.stream()
				.forEach((httpClientRewriteRule) -> map.put(
						httpClientRewriteRule.getMatch(),
						httpClientRewriteRule.getRouteToUri()));
