...
/**
 * A {@link WebFilter} that creates / continues / closes and detaches spans for a reactive web application.
 *
...
public final class TraceWebFilter implements WebFilter, Ordered {
    ...
    private static final Log                             log                       = LogFactory.getLog(org.springframework.cloud.sleuth.instrument.web.TraceWebFilter.class);

    private static final String                          HTTP_COMPONENT            = "http";
    protected static final String                        TRACE_REQUEST_ATTR        = org.springframework.cloud.sleuth.instrument.web.TraceWebFilter.class.getName()
                                                                                     + ".TRACE";
    private static final String                          TRACE_SPAN_WITHOUT_PARENT = org.springframework.cloud.sleuth.instrument.web.TraceWebFilter.class.getName()
                                                                                     + ".SPAN_WITH_NO_PARENT";

    /**
     * If you register your filter before the {@link org.springframework.cloud.sleuth.instrument.web.TraceWebFilter}
     * then you will not have the tracing context passed for you out of the box. That means that e.g. your logs will not
     * get correlated.
     */
    ...
    static final Propagation.Getter<HttpHeaders, String> GETTER                    = new Propagation.Getter<HttpHeaders, String>() {
                                                                                       ...
                                                                                       @Override
                                                                                       public String get(HttpHeaders carrier,
                                                                                                         String key) {
    ...
    public static TraceWebFilter create(BeanFactory beanFactory, SkipUriMatcher skipUriMatcher) {
    ...
    TraceContext.Extractor<HttpHeaders> extractor() {
        if (this.extractor == null) {
            this.extractor = this.beanFactory.getBean(HttpTracing.class).tracing().propagation().extractor(GETTER);
        }
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        return chain.filter(exchange).compose(f -> f.then(Mono.subscriberContext()).onErrorResume(t -> Mono.subscriberContext().map(c -> c.put(CONTEXT_ERROR,
                                                                                                                                               t))).flatMap(c -> {
                                                                                                                                                   // reactivate
                                                                                                                                                   // span
                                                                                                                                                   // from
                                                                                                                                                   ...
                                                                                                                                                   if (c.hasKey(CONTEXT_ERROR)) {
                                                                                                                                                       ...
                                                                                                                                                       if (attribute instanceof HandlerMethod) {
                                                                                                                                                           ...
                                                                                                                                                           addClassMethodTag(handlerMethod,
                                                                                                                                                                             span);
                                                                                                                                                           addClassNameTag(handlerMethod,
                                                                                                                                                                           span);
                                                                                                                                                       }
                                                                                                                                                   ...
                                                                                                                                                   if (c.hasKey(Span.class)) {
                                                                                                                                                       ...
                                                                                                                                                       if (log.isDebugEnabled()) {
                                                                                                                                                           log.debug("Found span in reactor context"
                                                                                                                                                                     + span);
                                                                                                                                                       }
                                                                                                                                                   } else {
                                                                                                                                                       try {
                                                                                                                                                           boolean hasTracingContextInHeaders = extractor().extract(request.getHeaders()) != TraceContextOrSamplingFlags.EMPTY;
                                                                                                                                                           // if
                                                                                                                                                           // there
                                                                                                                                                           // was
                                                                                                                                                           // a
    ...
    private Span unsampledSpan(String name) {
        Span span = tracer().nextSpan(TraceContextOrSamplingFlags.create(SamplingFlags.NOT_SAMPLED)).name(name).kind(Span.Kind.SERVER).start();
        if (log.isDebugEnabled()) {
    ...
    static final class HttpAdapter extends brave.http.HttpServerAdapter<ServerHttpRequest, ServerHttpResponse> {
